...
- Next step: rebuild/restart the backend and rerun the same manual query. Expected outcome is vector hits remain non-zero and low-scored but present paper evidence is shown as weak local evidence rather than refusal. After that, continue with Agent query-budget and zero-hit suppression work before changing UI.

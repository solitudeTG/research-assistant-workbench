...
F018.6 focused backend re-run on 2026-05-14:
...
F018.8 RED/GREEN reset cycle on 2026-05-15:

RED result:

```text
Compilation failed because EvidenceGateAgent and HeuristicEvidenceGateAgent did not exist.
```

Focused GREEN re-run:

```powershell
& 'C:\Users\HUAWEI\.m2\wrapper\dists\apache-maven-3.9.14\ed7edd442f634ac1c1ef5ba2b61b6d690b5221091f1a8e1123f5fadcc967520d\bin\mvn.cmd' '-Dtest=MultiAgentSubagentTest,MultiAgentPlanExecuteLoopTest' test
```
...
F018.8 report-boundary RED/GREEN on 2026-05-15:

```powershell
& 'C:\Users\HUAWEI\.m2\wrapper\dists\apache-maven-3.9.14\ed7edd442f634ac1c1ef5ba2b61b6d690b5221091f1a8e1123f5fadcc967520d\bin\mvn.cmd' '-Dtest=MultiAgentSubagentTest#documentComposerPassingOutputDoesNotExposeInternalEvidenceGateAccounting' test
```

RED result:

```text
Report exposed raw internal gap strings such as `Excluded 2 raw evidence candidate during hygiene filtering.` and `Rejected 3 evidence candidate during semantic gate.`
```
...
F018.8 broader backend regression on 2026-05-15:
...
F018.8 frontend model re-run on 2026-05-15:
...
- F018.8 removes deterministic topic-keyword promotion from the current design. The production evidence gate is model-backed and conservatively rejects all candidates if it cannot parse a gate response, so a failed gate may produce weak/no-evidence output rather than leaking unrelated evidence.
